...
def find_ffmpeg_binary(custom_path: Optional[str] = None) -> Optional[str]:
    """Find ffmpeg.exe path across bundled locations, custom settings, or system PATH."""
    # 1. Custom user path
    ...
    # 3. Next to executable or inside _internal
    if getattr(sys, 'frozen', False):
...
def find_ffprobe_binary(custom_path: Optional[str] = None) -> Optional[str]:
    """Find ffprobe.exe path across bundled locations, custom settings, or system PATH."""
    if custom_path and os.path.isfile(custom_path):
        return os.path.abspath(custom_path)
    if custom_path and os.path.isdir(custom_path):
